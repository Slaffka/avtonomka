...
$completeid = optional_param('completeid', 0, PARAM_INT);
// end custom version
        ...
        if (!$students) {
            ...
            foreach ($students as $student) {
                ...
                if ($completed_count > 0) {
                    ...
                    // begin custom version
                    $showentryurl_params = array('userid'=>$student->id, 'do_show'=>'showoneentry', 'completeid'=>$student->completeid);
                    $showentryurl = new moodle_url($url, $showentryurl_params);
                    $showentrylink = '<a href="'.$showentryurl->out().'">'.userdate($student->completed_timemodified).'</a>';
                    // end custom version
                    $data[] = $showentrylink;
