...
function feedback_get_complete_users($cm,
    ...
    $sql = 'SELECT  c.id as completeid, '.$ufields.', c.timemodified as completed_timemodified
 ...
 */
// begin custom version
function feedback_get_receivemail_users($cmid, $type, $groups = false) {
// end custom version
    $context = context_module::instance($cmid);
    ...
    //                          $limitnum='', $groups='', $exceptions='', $doanything=true)
    // begin custom version
    $cap = 'mod/feedback:receivemail';
    switch($type){
        case 'qualitycheck':
            $cap = 'mod/feedback:receivemail_checkquality';
            break;
        case 'mysteryshopper':
            $cap = 'mod/feedback:receivemail_mysteryshopper';
            break;
...
function feedback_send_email($cm, $feedback, $course, $userid) {
    ...
    if ($groupmode == SEPARATEGROUPS) {
        ...
        $teachers = feedback_get_receivemail_users($cm->id, $feedback->type, $groups);
    } else {
        $teachers = feedback_get_receivemail_users($cm->id, $feedback->type);
    }
    // end custom version
...
function feedback_send_email_text($info, $course) {
    ...
    switch($info->type){
        case 'mysteryshopper':
            $posttext = get_string("emailmysteryshoppertext", "feedback", $info);
            break;

        default:
        case 'feedback':
        case 'qualitycheck':
            $posttext = get_string("emailfeedbacktext", "feedback", $info);
            break;
    }
...
function feedback_send_email_html($info, $course, $cm) {
    ...
    switch($info->type){
        case 'mysteryshopper':
            $posthtml = get_string('emailmysteryshopperhtml', 'feedback', $info);
            break;

        default:
        case 'feedback':
        case 'qualitycheck':
            $posthtml = get_string('emailfeedbackhtml', 'feedback', $info);
            break;
    }
    // end custom version
