...
(function ($) {
    ...
    function GTreeTable(element, options) {
        ...
        this.template = this.options.template !== undefined ? this.options.template : 
           ...
           '<td>' +
           '<span>${draggableIcon}${indent}${ecIcon}${selectedIcon}${typeIcon}${divisionname}${post} - ${name}, ${city}, функ.рук.:${funcdir}, тренер: ${trname}</span>' +
      ...
      this.templateParts = this.options.templateParts !== undefined ? this.options.templateParts :
            {
                ...
                name: '<a class="' + this.options.classes.name + '"></a>',
                ...
                actionsList: '',
                post: '<span class="'+this.options.classes.post + '"></span>',
                city: '<span class="'+this.options.classes.city + '"></span>',
                funcdir: '<span class="'+this.options.classes.funcdir + '"></span>',
                divisionname: '<span class="'+this.options.classes.divisionname + '"></span>',
                trname: '<span class="'+this.options.classes.trname + '"></span>'
            };
    ...
    GTreeTableNode.prototype = {
        ...
        init: function () {
            ...
            this.$post = this.$node.find('.' + this.manager.options.classes.post);
            this.$city = this.$node.find('.' + this.manager.options.classes.city);
            this.$funcdir = this.$node.find('.' + this.manager.options.classes.funcdir);
            this.$divisionname = this.$node.find('.' + this.manager.options.classes.divisionname);
            this.$trname = this.$node.find('.' + this.manager.options.classes.trname);
        ...
        render: function() {
            ...
            this.$name.attr("href", this.ref);

            this.$funcdir.html(this.funcdirname || "нет");

            this.$post.html(this.post || "Не указана должность");

            this.$city.html(this.city || "Не указан город");

            this.$divisionname.html(this.divisionname || "");

            this.$trname.html(this.trname || "нет");
        ...
        attachEvents: function () {
            ...
            if (isNaN(selectLimit) === false && (selectLimit > 0 || selectLimit === -1) ) {
                ...
                this.$post.click(function (e) { that.$ceIcon.click(); });
            }
        ...
        showCeIcon: function (action) {
            ...
            action ? this.$ceIcon.css('visibility', 'visible') : alert("Поиск не дал результатов в одной из секций");
        },
    ...
    $.fn.gtreetable.defaults = {
        ...
        languages: {
            ...
            'ru':{
                ...
                action: 'Опции',
                actions: {
                    //createBefore: 'Создать перед...',
                    createAfter: 'Создать соседа',
                    //createFirstChild: 'Создать первого потомка',
                    createLastChild: 'Создать потомка',
                    update: 'Редактировать',
                    'delete': 'Удалить'
                },
                messages: {
                    onDelete: 'Вы уверены?',
                    onNewRootNotAllowed: 'Добавление корневого узла не допускается.',
                    onMoveInDescendant: 'Цель не может быть потомком.',
                    onMoveAsRoot: 'Цель не может быть корнем.'
                }
            }
        },
        defaultActions: [
            ...
            */
            {
                ...
                event: function (oNode, oManager) {
                    ...
                    $("#new_parent_id").val(oNode.parent);
                    $("#new_company_id").val(oNode.partnerid);
                    $("#addstaff-modal").css("visibility", "visible");
                }
            },
            /*{
            ...
            */
            ...
            },
            {
                ...
                event: function (oNode, oManager) {
                    ...
                    $("#changes_type").val("update");

                    var $edit_window = $("#edit_window");
                    $edit_window.find("select").prop('disabled', false);

                    $.getJSON('/blocks/manage/?__ajc=orgstructure::get_nodes',
                        {
                            id_edit: oNode.id
                        }
                    )
                        .done(function(data) {
                            if(data.length) {
                                $("#select_user option[value='" + data[0].ref + "']").prop("selected", true);
                                $("#select_post2 option[value='" + data[0].postid + "']").prop("selected", true);
                                $("#select_region2 option[value='" + data[0].cityid + "']").prop("selected", true);
                                $("#select_distrib2 option[value='" + data[0].distribid + "']").prop("selected", true);
                                $("#select_segment2 option[value='" + data[0].segmentid + "']").prop("selected", true);
                                $("#select_user2 option[value='" + data[0].parentfid + "']").prop("selected", true);
                                $("#select_place option[value='" + data[0].areaid + "']").prop("selected", true);


                                if (data[0].code) {
                                    //защитить от редактирования (разрешено только подразделение и функ рук)
                                    $("#select_user, #select_region2, #select_distrib2, #select_place, #select_segment2, #select_post2").prop('disabled', 'disabled');
                                }
                                $edit_window.find("select").trigger("chosen:updated");
                                $("#select_user_chosen").css('width','94%');
                                show_window_edit();
                            }
                        });
                }
