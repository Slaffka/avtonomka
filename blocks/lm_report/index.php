...
foreach ($res as $key => $val) {
    ...
    $cheif = enterEmpty($DB->get_record_sql("SELECT CONCAT_WS(' ', u.lastname, u.firstname) as name FROM mdl_user as u2 INNER JOIN (mdl_lm_position_xref as px2 INNER JOIN (mdl_lm_position as p INNER JOIN (mdl_user as u INNER JOIN mdl_lm_position_xref as px ON u.id = px.userid) ON p.parentid = px.posid) ON px2.id = p.id) ON u2.id = px2.userid AND u2.id = " . $val['userid']));
