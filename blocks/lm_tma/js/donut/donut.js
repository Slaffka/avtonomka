(function (root, factory) {
	...
	var donut = function( options ) {
		...
		var base_size = 140,// Базовый размер графика, для которого норма размера текста 16px
			scale_k = size/base_size; // Коэфициент изменения размера шрифта

		lbl_font_size = scale_k*lbl_font_size;

		if( datalbl ) {
			arc = $(doc.createElementNS(NS, 'text'));
			arc.attr({x: '50%', y: '95%', style: 'text-anchor: middle;font-size:'+lbl_font_size+'px', fill:"#01416a"})
				.text( datalbl ).appendTo(svg);
		}
		...
		lbl.style['line-height'] = size + 'px';
		lbl.style['text-align'] = 'center';
		lbl.style['font-size'] = lbl_font_size + 'px';
		lbl.style['font-weight'] = 'bold';

		// Тип графика полукруг
		if( type == "semi-circle" ) {
			lbl.style['line-height'] = lbl_font_size + 'px';
			lbl.style['top'] = ((size/2 - lbl_font_size)/100 * 65) + 'px';
		}
